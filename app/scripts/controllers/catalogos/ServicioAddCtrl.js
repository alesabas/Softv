  ...
  .controller("ServicioAddCtrl", function(
      ...
      CatalogosFactory.GetDeepTipServ_New(vm.Clv_TipSer).then(function(data) {
        ...
        if (TipoServicioResult != null) {
          if (TipoServicioResult.Habilitar == 0) {
            ...
            CatalogosFactory.GetMUESTRATRABAJOS_NewList(vm.Clv_TipSer).then(
              function(data) {
                vm.TrabajoList = data.GetMUESTRATRABAJOS_NewListResult;
              }
            );
            CatalogosFactory.GetTipoClienteList_WebSoftvnew().then(function(
              data
            ) {
            ...
            ngNotify.set(
              "ERROR, el tipo se servicio que seleccionó no es válido.",
              "warn"
            );
    ...
    function SaveServicios() {
      ...
      CatalogosFactory.AddServicios_New(objServicios_New).then(function(data) {
        ...
        if (Clv_Servicio > 0) {
          CatalogosFactory.DeleteBORRel_Trabajos_NoCobroMensual(
            Clv_Servicio
          ).then(function(data) {
            ...
            if (BorrarTrabajo == -1) {
              if (vm.GeneraOrden == "Y") {
                ...
                CatalogosFactory.UpdateGUARDARel_Trabajos_NoCobroMensual(
                  objGUARDARel_Trabajos_NoCobroMensual
                ).then(function(data) {
                  if (
                    data.UpdateGUARDARel_Trabajos_NoCobroMensualResult == -1
                  ) {
                    if (vm.Clv_TipSer == 2) {
                      ...
                      ngNotify.set(
                        "CORRECTO, se añadió un servicio nuevo.",
                        "success"
                      );
                      $state.go("home.catalogos.servicio_editar", {
                        id: vm.Clv_Servicio
                      });
                    }
                  } else {
                ...
                CatalogosFactory.AddNUEPuntos_Pago_Adelantado(
                  objNUEPuntos_Pago_Adelantado
                ).then(function(data) {
                  if (vm.Clv_TipSer == 2) {
                    if (vm.ClvEquiListForm.length > 0) {
                      ...
                      ServiciosFactory.GetAddServicioClvEqMedio(
                        ObjClvEquivalente
                      ).then(function(data) {
                        SaveServicio2(Clv_Servicio);
                      });
          ...
          ngNotify.set(
            "ERROR, al añadir un servicio nuevo, posiblemente la clave que ingresó ya existe.",
            "warn"
          );
        }
      });
    ...
    function SaveServicio2(Clv_Servicio) {
      ...
      CatalogosFactory.AddValidaAplicaSoloInternet(
        objValidaAplicaSoloInternet
      ).then(function(data) {
        ...
        if (ValildaInternetResult == 0) {
          ...
          CatalogosFactory.AddNueAplicaSoloInternet(
            objNueAplicaSoloInternet
          ).then(function(data) {
          ...
          CatalogosFactory.DeleteBorAplicaSoloInternet(Clv_Servicio).then(
            function(data) {
              ngNotify.set("CORRECTO, se añadió un servicio nuevo.", "success");
              $state.go("home.catalogos.servicio_editar", { id: Clv_Servicio });
            }
    ...
    function SaveServicio3(Clv_Servicio) {
      ...
      CatalogosFactory.UpdateNUEVOClv_Equi(objNUEVOClv_Equi).then(function(
        data
      ) {
        if (data.UpdateNUEVOClv_EquiResult == 1) {
          ...
          ngNotify.set(
            "ERROR, al agregar uivalente al servicio nuevo.",
            "warn"
          );
          $state.go("home.catalogos.servicios");
        }
    ...
    function GetTarifa() {
      if (vm.TipoCobro != undefined) {
        ...
        CatalogosFactory.GetREL_TARIFADOS_SERVICIOS_NewList(ObjTarifa).then(
          function(data) {
