...
angular
  .module("softvApp")
  .controller("FacturacionCajasCtrl", function(
    $uibModal,
    $state,
    $rootScope,
    ...
    function openEdoCuenta() {
      cajasFactory.getEstadoCuenta(vm.Cliente.Contrato).then(function(data) {
        if (data.GetDeeptieneEdoCuentaResult.tieneEdoCuenta) {
          ...
          ngNotify.set(
            "El cliente no cuenta con estado de cuenta generado.",
            "info"
          );
        }
      });
        ...
        if (vm.selectAparato != "") {
          cajasFactory
            .quitarDetalle(
              vm.Cliente.Contrato,
              vm.session,
              vm.selectAparato.CLV_DETALLE,
              vm.Suscriptor.Clv_TipoCliente
            )
            .then(function(data) {
              if (data.GetQuitarDetalleListResult[0].Error == 0) {
                cajasFactory
                  .dameDetallePago(vm.session)
                  .then(function(detallePago) {
                ...
                cajasFactory
                  .addBitacora(
                    vm.Cliente.Contrato,
                    vm.selectAparato.Pagos_Adelantado
                  )
                  .then(function(data) {});
    ...
    function adelantaPagos(item) {
      cajasFactory.puedoAdelantarPago(vm.session).then(function(data) {
        ...
        if (vm.errPag == 0) {
          cajasFactory
            .checaAdelantarPagos(vm.Cliente.Contrato)
            .then(function(dataCheca) {
              ...
              if (vm.errChec == 0) {
                ...
                ngNotify.set(
                  dataCheca.GetDeepChecaAdelantarPagosDifResult.Msg,
                  "info"
                );
              }
            });
    ...
    function openSuspencion() {
      vm.selectAparato = "";
      cajasFactory
        .suspencionTemporal(vm.Cliente.Contrato, vm.session)
        .then(function(data) {
    ...
    function openPay(tipo) {
      ...
      cajasFactory.dameSucursalCompa(vm.Cliente.Contrato).then(function(data) {
        if (data.GetDeepDameRelSucursalCompaResult.Id == 0) {
          ngNotify.set(
            "La caja no tiene asignados folios para esta plaza.",
            "error"
          );
        } else {
          cajasFactory
            .dimeSiYaFact(vm.Cliente.Contrato)
            .then(function(dataDime) {
              if (dataDime.GetDeepDimeSiYaGrabeFacResult.Id == 0) {
                cajasFactory
                  .sumaTotalDetalle(vm.session)
                  .then(function(sumaTotal) {
                    var items = {
    ...
    function openReturn() {
      if (vm.selectAparato == undefined) {
        ...
        ngNotify.set(
          "Solo se puede cobrar adeudos a conceptos de mensualidad.",
          "error"
        );
      } else {
        vm.elaparato = "";
        ...
        if (vm.labandera == false) {
          ngNotify.set(
            "Debe haber conceptos de Mensualidad con servicio Instalado por cobrar para aplicar cobro de adeudo.",
            "error"
          );
        } else {
          vm.items = {
    ...
    function openClabe() {
      cajasFactory.dameClabe(vm.Cliente.Contrato).then(function(data) {
        if (data.GetBotonClabeListResult[0].YaTiene == 0) {
          ngNotify.set(
            "El contrato no cuenta con Cuenta Clabe asignada.",
            "info"
          );
        } else {
          var items = {
    ...
    function buscarPorContrato(contratoForm) {
      ...
      cajasFactory
        .validarContrato(vm.data.contrato)
        .then(function(datacontrato) {
          if (
            datacontrato.Getsp_dameContratoCompaniaAdicListResult[0].Contrato >
            0
          ) {
            cajasFactory.buscarContrato(contrato).then(function(data) {
              if (data.GetBusCliPorContrato_FacListResult.length > 0) {
                ...
                cajasFactory
                  .ValidaSaldoContrato(vm.Cliente.Contrato)
                  .then(function(data) {
                    if (data.GetValidaSaldoContratoResult.tieneSaldo > 0) {
                      vm.ArrastraSaldo = true;
                      cajasFactory
                        .CobraSaldo(vm.Cliente.Contrato)
                        .then(function(cobra) {
                          vm.session = cobra.GetDeepCobraSaldoResult.ClvSession;
                          cajasFactory
                            .preguntaCajas(vm.Cliente.Contrato, 0)
                            .then(function(op1) {
                              if (
                                op1.GetDeepuspHaz_PreguntaResult.Pregunta !=
                                null
                              ) {
                                abrirModalPregunta(
                                  0,
                                  op1.GetDeepuspHaz_PreguntaResult.Pregunta,
                                  op1.GetDeepuspHaz_PreguntaResult
                                    .MesesAdelantados
                                );
                              }
                            });
                          ...
                          cajasFactory
                            .preguntaCajas(vm.Cliente.Contrato, 2)
                            .then(function(op1) {
                              if (
                                op1.GetDeepuspHaz_PreguntaResult.Pregunta !=
                                null
                          ...
                          cajasFactory
                            .preguntaCajas(vm.Cliente.Contrato, 3)
                            .then(function(op1) {
                              if (
                                op1.GetDeepuspHaz_PreguntaResult.Pregunta !=
                                null
                          ...
                          cajasFactory
                            .preguntaCajas(vm.Cliente.Contrato, 900)
                            .then(function(op1) {
                              if (
                                op1.GetDeepuspHaz_PreguntaResult.Pregunta !=
                                null
                          ...
                          cajasFactory
                            .getObservaciones(vm.Cliente.Contrato)
                            .then(function(observa) {
                              if (observa.GetDeepConRelClienteObsResult.Obs) {
                                new PNotify({
                                  title: "Observaciones",
                                  type: "info",
                                  text:
                                    observa.GetDeepConRelClienteObsResult.Obs,
                                  hide: false
                                });
                              }
                            });
                          reloadTables();
                          cajasFactory
                            .ObtieneEdoCuentaSinSaldar(
                              vm.Cliente.Contrato,
                              vm.session
                            )
                            .then(function(detalleEdo) {
                              vm.detalleEdo =
                                detalleEdo.GetObtieneEdoCuentaSinSaldarListResult;
                            });
                        });
                    } else {
                      cajasFactory
                        .dameSession(vm.Cliente.Contrato)
                        .then(function(session) {
                          vm.session =
                            session.GetDeepDameClv_SessionResult.IdSession;
                          cajasFactory
                            .preguntaCajas(vm.Cliente.Contrato, 0)
                            .then(function(op1) {
                              if (
                          ...
                          if (session.GetDeepDameClv_SessionResult.Error == 0) {
                            ...
                            ngNotify.set(
                              session.GetDeepDameClv_SessionResult.Msg,
                              {
                                type: "warn",
                                sticky: true
                              }
                ...
                cajasFactory
                  .checaRetiro(vm.Cliente.Contrato)
                ...
                cajasFactory
                  .serviciosCliente(vm.Cliente.Contrato)
                  .then(function(servicios) {
                    vm.servicios = servicios.GetDameSerDelCliFacListResult;
                  });
                cajasFactory
                  .dameSuscriptor(vm.Cliente.Contrato)
                  .then(function(suscriptor) {
                    vm.Suscriptor =
                      suscriptor.GetDameTiposClientesListResult[0];
                  });
                cajasFactory
                  .damePeriodoCliente(vm.Cliente.Contrato)
                  .then(function(dataPeriodo) {
            ...
            ngNotify.set(
              "El usuario no tiene permisos para ver a este cliente ó el contrato no existe.",
              "error"
            );
            reset();
          }
    ...
    function buscarPorNombre() {
      ...
      cajasFactory
        .buscarPorNombre(vm.data.nombre, vm.data.apaterno, vm.data.amaterno)
        .then(function(data) {
    ...
    function selectCliente(x) {
      ...
      cajasFactory.validarContrato(x).then(function(datacontrato) {
        if (
          datacontrato.Getsp_dameContratoCompaniaAdicListResult[0].Contrato > 0
        ) {
          cajasFactory.buscarContrato(x).then(function(data) {
            if (data.GetBusCliPorContrato_FacListResult.length > 0) {
                  ...
                  if (data.GetValidaSaldoContratoResult.tieneSaldo > 0) {
                    ...
                    cajasFactory
                      ...
                      .then(function(cobra) {
                        ...
                        cajasFactory
                          ...
                          .then(function(op1) {
                            if (
                              op1.GetDeepuspHaz_PreguntaResult.Pregunta != null
                            ) {
                              abrirModalPregunta(
                                0,
                                op1.GetDeepuspHaz_PreguntaResult.Pregunta,
                    ...
                    cajasFactory
                      ...
                      .then(function(session) {
                        ...
                        cajasFactory
                          .ValidaSaldoContrato(vm.Cliente.Contrato, vm.session)
                          .then(function(data) {
                            if (
                              data.GetValidaSaldoContratoResult.tieneSaldo > 0
                            ) {
                              vm.ArrastraSaldo = true;
                            }
                          });
                        ...
                        cajasFactory
                          ...
                          .then(function(retiro) {
                            if (
                              retiro.GetChecaOrdenRetiroListResult[0]
                                .Resultado > 0
                            ) {
                              new PNotify({
                                title: "Aviso",
                                text:
                                  retiro.GetChecaOrdenRetiroListResult[0].Msg,
                                hide: false
                              });
                            }
                          });
              ...
              ngNotify.set(
                "No se encontro ningun cliente con ese número de contrato.",
                "error"
              );
              reset();
            }
          ...
          ngNotify.set(
            "El usuario no tiene permisos para ver a este cliente.",
            "error"
          );
          reset();
        }
    ...
    function buscarPorDomicilio() {
      ...
      vm.isCollapsed = !vm.isCollapsed;
      cajasFactory
        .buscarPorDireccion(vm.data.calle, vm.data.numero)
        .then(function(data) {
          if (data.GetuspBusCliPorContratoSeparadoListResult.length == 0) {
            ngNotify.set("No se encontro ninguna coincidencia.", "error");
            reset();
