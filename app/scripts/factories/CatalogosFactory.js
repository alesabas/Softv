...
angular
  ...
  .factory('CatalogosFactory', function ($http, $q, globalService, $localStorage) {
    ...
    factory.GetDeepRel_Trabajos_NoCobroMensual = function (Clv_Servicio) {
      var deferred = $q.defer();
      var config = {headers: {'Authorization': $localStorage.currentUser.token}};
    ...
    factory.GetCatMediosList = function () {
      var deferred = $q.defer();
      var config = {headers: {'Authorization': $localStorage.currentUser.token}};
      $http.get(globalService.getUrl() + paths.GetCatMediosList, config).then(function (response) {
        deferred.resolve(response.data);
      }).catch(function (response) {
        deferred.reject(response);
      });
